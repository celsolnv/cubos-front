...
export const movieSchema = z.object({
  ...
  popularity: f.numberTransform("Popularidade"),
  votes: f.numberTransform("Votos"),
  duration: f.numberTransform("Duração em minutos"),
  genres: z.array(z.string()).transform((val) => val.map((v) => v.trim())),

  // TODO: validar generos, imagem, status
});
